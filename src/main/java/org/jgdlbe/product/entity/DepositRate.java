...
public class DepositRate {
    ...
    @Column(name = "int_rate_type", length = 1)
    private String intRateType;    // 'S'단리 / 'M'복리
    @Column(name = "intr_rate_type_nm")
    private String intrRateTypeNm; // 금리유형명
    @Column(name = "save_trm")
    private Short saveTrm;         // 저축기간(개월)
    @Column(name = "intr_rate", precision = 5, scale = 3)
    private BigDecimal intrRate;   // 기본금리
    @Column(name = "intr_rate2", precision = 5, scale = 3)
    private BigDecimal intrRate2;  // 최고우대금리
    @Column(name = "rsrv_type", length = 8, nullable = false)
    private String rsrvType;     // S=정액적립식, F=자유적립식, 예금=NA
